...
function encrypt (text) {
  ...
  const buffer = Buffer.concat([encrypted, cipher.final(), iv]).toString('base64')
  return buffer
}
...
function decrypt (text) {
  const key = process.env.CRYPTO_KEY
  const ivLength = 16
  const bynaryText = Buffer.from(text, 'base64')
  const encryptedLength = bynaryText.length - ivLength
  const iv = bynaryText.slice(encryptedLength)
  const encryptedText = bynaryText.slice(0, encryptedLength)
  const decipher = crypto.createDecipheriv('aes-256-cbc', Buffer.from(key), iv)
  const decrypted = decipher.update(encryptedText)
  const buffer = Buffer.concat([decrypted, decipher.final()]).toString()
  return buffer
}
